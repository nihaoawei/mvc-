...
<style>
	...
	<div class="tzgl" style="float:right">
			...
			<p style="font-size: 15px;font-weight: bold;margin:20px;">主题数<?=$huiCount;?></p>
				<div class="line clearFix">
				<ul  class="li">
					<li style="float: left;list-style: none;width:170px;" class="two jiacu">作者</li>

					<li style="float: left;list-style: none;width:170px;" class="two jiacu">回复内容</li>
					<li style="float: left;list-style: none;width:170px;" class="four jiacu">回复时间</li>
				</ul>
				</div>
				<form action="index.php?m=guanli&a=huiGuan" method="get">

				<input class="one" type="hidden" name="m" value="guanli" /><br/>
				<input class="one" type="hidden" name="a" value="huiGuan" /><br/>
				<?php if(!empty($hui)): ?>
				<?php foreach($hui as $key=>$val) :?>
					<p class="tiezigl">
						<input type="checkbox" name="id[]" value="<?=$val['yid'];?>" />
						<em  style="width:170px;display: inline-block;" class="em2"><?=$val['uname'];?></em>
						<em  style="width:170px; height:20px ;display: inline-block; overflow: hidden;" class="em2"><?php echo strip_tags($val['content']);?></em>
				...
				<?php endif;?>
					<br/>
					<input type="submit" name="del" value="删除回复">
				</form>
			</div>
		</div>
</div>
